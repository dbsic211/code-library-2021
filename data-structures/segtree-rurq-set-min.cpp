...
struct segtree_rurq { 
    // Range update range query, standard form 3 (assignment/min)
    struct node {
        long long upd = 0;
        long long ans = 0;
        bool exist = 0;
    ...
    void u(int l, int r, int constl, int constr, int idx, long long val) {
        ...
        if(a[idx].exist == 1) {
            ...
            a[2*idx+2].exist = 1;
            a[idx].exist = 0;
            a[2*idx+1].ans = a[2*idx+1].upd;
            a[2*idx+2].ans = a[2*idx+2].upd;
            a[idx].ans = min(a[2*idx+1].ans, a[2*idx+2].ans);
        }
    ...
    long long qu(int l, int r, int constl, int constr, int idx) {
        ...
        if(mid<l || r<constl) { 
            ...
            ret = min(qu(l, r, constl, mid, 2*idx+1), qu(l, r, mid+1, constr, 2*idx+2));
        }
        if(mod) ret %= mod;
        return ret;
    }
